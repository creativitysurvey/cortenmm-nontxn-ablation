...
impl<'a, C: PageTableConfig> Cursor<'a, C> {
    ...
    >)
        ...
        ensures
            ...
            forall|level: PagingLevel|
                self.guard_level <= level <= C::NR_LEVELS() ==>
                    #[trigger] self.get_guard_nid(level) =~= old(self).get_guard_nid(level),
    {
        ...
        loop
            invariant
                ...
                forall|level: PagingLevel|
                    self.guard_level <= level <= C::NR_LEVELS() ==>
                        #[trigger] self.get_guard_level(level) =~= old(self).get_guard_level(level),
                m.inv(),
                m.inst_id() == old(self).inst@.id(),
                m.state() is WriteLocked,
                m.sub_tree_rt() =~= old(self).get_guard_nid(old(self).guard_level),
            decreases self.level,
        {
            ...
            match child {
                ChildRef::PageTable(pt) => {
                    ...
                    assert forall|lvl: PagingLevel|
                        #![trigger _cursor.get_guard_level(lvl)]
                        self.guard_level <= lvl <= C::NR_LEVELS()
                    implies {
                        _cursor.get_guard_level(lvl) =~= self.get_guard_level(lvl)
                    } by {};
                    assert forall|lvl: PagingLevel|
                        #![trigger self.get_guard_level(lvl)]
                        self.guard_level <= lvl <= C::NR_LEVELS()
                    implies {
                        old(self).get_guard_level(lvl) =~= self.get_guard_level(lvl)
                    } by {
                        assert(old(self).get_guard_level(lvl) =~= _cursor.get_guard_level(lvl));
                        assert(_cursor.get_guard_level(lvl) =~= self.get_guard_level(lvl));
                    };
                    continue ;
                },
            ...
            self.g_level@ == self.level,
    {
        ...
        let ghost idx = (self.level - 1) as int;
        self.put_guard((self.level - 1) as usize, new_guard);
        assert forall|level: PagingLevel|
            #![trigger _cursor.get_guard_level(level)]
            self.guard_level <= level <= C::NR_LEVELS()
        implies {
            _cursor.get_guard_level(level) =~= self.get_guard_level(level)
        } by {
            assert(idx < self.guard_level - 1);
...
impl<'a, C: PageTableConfig> CursorMut<'a, C> {
    ...
    pub fn map(&mut self, item: C::Item, Tracked(m): Tracked<&LockProtocolModel<C>>) -> (res:
        ...
        ensures
            ...
            forall|level: PagingLevel|
                self.0.guard_level <= level <= C::NR_LEVELS() ==>
                    #[trigger] self.0.get_guard_nid(level) =~= old(self).0.get_guard_nid(level),
    {
        broadcast use Cursor::group_cursor_lemmas;
            ...
            decreases self.0.level,
        {
            ...
            match child {
                ChildRef::PageTable(pt) => {
                    ...
                    assert forall|lvl: PagingLevel|
                        #![trigger _cursor.get_guard_nid(lvl)]
                ...
                ChildRef::None => {
                    ...
                    self.0.push_level(child_pt);
                    assert forall|lvl: PagingLevel|
                        #![trigger _mid.get_guard_level(lvl)]
                        self.0.guard_level <= lvl <= C::NR_LEVELS()
                    implies {
                        _mid.get_guard_level(lvl) =~= self.0.get_guard_level(lvl)
                    } by {};
                    assert forall|lvl: PagingLevel|
                        #![trigger self.0.get_guard_nid(lvl)]
                        self.0.guard_level <= lvl <= C::NR_LEVELS()
                    implies {
                        ...
                        assert(old(self).0.get_guard_nid(lvl) =~= _cursor.get_guard_nid(lvl));
                        assert(_cursor.get_guard_nid(lvl) =~= _mid.get_guard_nid(lvl));
                        assert(_mid.get_guard_nid(lvl) =~= self.0.get_guard_nid(lvl));
                    };
                },
        ...
        implies {
            ...
            assert(old(self).0.get_guard_nid(lvl) =~= _cursor.get_guard_nid(lvl));
            assert(_cursor.get_guard_nid(lvl) =~= _pre_move.get_guard_nid(lvl));
            assert(_pre_move.get_guard_level(lvl) =~= self.0.get_guard_level(lvl));
        };
                    ...
                    #[trigger] self.0.get_guard_nid(level) =~= old(self).0.get_guard_nid(level),
    {
                    ...
                    self.0.level,
        {
                    ...
                    ChildRef::PageTable(pt) => {
                        ...
                        if pt.nr_children() != 0 {
                            ...
                            implies {
                                ...
                                assert(old(self).0.get_guard_nid(lvl) =~= _cursor2.get_guard_nid(lvl));
                            };
                        }
                    },
                    ChildRef::None => {
            ...
            | GuardInPath::Write(mut cur_node)) = guard_in_path else { unreached() };
            let ghost _cur_node = cur_node;
            let ghost outer_old_self = old(self).0;
            ...
            assert forall|lvl: PagingLevel|
                ...
                self.0.guard_level <= lvl <= C::NR_LEVELS()
            implies {
                outer_old_self.get_guard_nid(lvl) =~= self.0.get_guard_nid(lvl)
            } by {
                assert(outer_old_self.get_guard_nid(lvl) =~= _cursor.get_guard_nid(lvl));
                assert(_cursor.get_guard_nid(lvl) =~= _pre_move.get_guard_nid(lvl));
                assert(_pre_move.get_guard_level(lvl) =~= self.0.get_guard_level(lvl));
            };
            return item;
