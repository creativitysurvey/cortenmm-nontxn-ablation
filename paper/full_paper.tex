...
\title{\bf The Locking-Gated Threshold (LGT) Effect:
...
count (160). We formalize this as the \emph{Locking-Gated Threshold
(LGT) effect} --- the cost is
...
transactional interfaces, ablation study, proof cost, Locking-Gated
Threshold (LGT) effect.
...
RQ2 (\S\ref{sec:eval:gap4}) &
  ...
  the Locking-Gated Threshold (LGT) effect,
...
\begin{claim}[Locking-Gated Threshold (LGT) effect]
...
We refer to this pattern throughout the rest of the paper as the LGT
effect: the frame-preservation obligation's \emph{existence} is gated
by the locking protocol's structure, not a continuously-varying cost
of the interface change itself.
...
\end{tikzpicture}
\caption{The LGT effect (Claim~\ref{claim:threshold}): whether the
...
cost.'' It is the LGT effect: the cost exists, and is substantial
...
LGT effect reappears, independently derived and independently repaired,
...
two residual issues). \emph{Post-submission reproduction note:} the
proof-cost reproduction path (\vfn{scripts/verify\_all\_variants.sh})
has since been independently confirmed to reproduce end-to-end (exit
code 0, all six variants' exact obligation/error counts as reported
above); the runtime-benchmark kernel-image build path
(\vfn{scripts/run\_benchmarks.sh}) has, in a separate reproduction
attempt, encountered a recurrence of the \vfn{cargo-osdk}
manifest-generation issue (\S\ref{sec:eval:threats}) that did not
respond to the remedy originally recorded for it; this is documented
in \vfn{docs/environment-setup-and-findings.md} as a currently
unresolved, environment-sensitive build issue affecting only the
runtime-benchmark reproduction path, not the paper's proof-cost claims
or the original runtime measurements themselves
(\vfn{benchmarks/raw\_data\_*.csv}). This artifact does not include a build of an
