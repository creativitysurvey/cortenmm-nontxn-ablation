...
explicit scope and non-claims --- including that two of our six
variants remain blocked from a clean 0-error result by a single,
documented Verus tooling limitation --- positioned within a four-axis
...
RQ3 (\S\ref{sec:eval:rcu}) &
  ...
  Both \corten{nontxn} and \corten{rcu-nontxn} are blocked from 0 errors
  by the same class of residual Verus tooling limitation
  (\S\ref{sec:eval:threats}), not by any logical gap. \\
\addlinespace
...
\begin{itemize}
  ...
  \item \emph{Verification-preserving ablation}
    ...
    0 errors; \corten{nontxn} and \corten{rcu-nontxn} have their core
    frame-preservation property fully proved but are blocked from a
    clean 0-error result only by a documented, reproducible tooling
    limitation (\S\ref{sec:eval:threats}), the same limitation in both
    cases (\S\ref{sec:method:variants}, \S\ref{sec:eval}).
  ...
  \item Four independent, reproducible artifact-infrastructure findings
    ...
    binary, a manifest-generation inconsistency across builds of the
    same OS-development-kit tool, and a corrected --- initially
...
Residual blocker & \texttt{InstanceId} equality (tooling) & \texttt{InstanceId} equality (tooling), same class \\
\bottomrule
\end{tabularx}
\end{table}
...
\paragraph{Residual Verus tooling limitation.} Both \corten{nontxn} and
\corten{rcu-nontxn} are blocked from a clean $0$-error verification by a
single, structurally identical residual issue: a manually re-derived
fact of the form \texttt{res.inst\_id() =\textasciitilde{}=
m.inst\_id()} fails to verify even though the same fact, checked via a
standalone \texttt{assert} immediately beforehand, succeeds; Verus's
own diagnostic further reports that the \emph{unwrapped} field
comparison (\texttt{res.inst\_id().0 =\textasciitilde{}=
m.inst\_id().0}) would succeed. We applied the two standard remedies
suggested by Verus's own troubleshooting guide
(\texttt{\#[verifier::rlimit(n)]} and
\texttt{\#[verifier::spinoff\_prover]}); neither resolves it. We report
this as an open, reproducible tooling limitation rather than a gap in
the ablation's logical content, since the corresponding frame-preservation
property --- the actual subject of RQ2/RQ3 --- is fully proved in every
case.
...
\begin{itemize}
  ...
  \item \textbf{No claim about the residual tooling issue.} The
    \vfn{InstanceId}-equality failure reported in
    \S\ref{sec:eval:threats} is, by our own diagnosis there, a property
    ...
    this section makes any prediction about it, and none should be
    inferred.
...
axis is available; resolving the residual \vfn{InstanceId}-equality
tooling limitation that keeps two of our six variants from a clean
...
0 verification errors for \corten{nontxn} or \corten{rcu-nontxn}; it
documents exactly the 2- and 3-error residual states \S\ref{sec:eval}
reports, with the specific failing precondition clause identified in
\vfn{docs/verus-tooling-issue.md}. That document also records a finer
distinction not stated elsewhere in the paper: \corten{nontxn}'s
\vfn{query\_locked} reaches a clean 0-error result, while
\corten{rcu-nontxn}'s does not (all three of its wrapper functions hit
the residual issue, not only \vfn{map\_locked}/\vfn{unmap\_locked}).
This artifact does not include a build of an
